...
router.get('/', function (req, res) {
    setting.findById('5ec23c3d5afd4804788a14e5', function(err, setting){
...
router.post('/save-setting', function (req, res) {
    ...
    config.jwt.verify(token, config.secretToken, function (err, decoded) {
        ...
        var filter = { _id: '5ec23c3d5afd4804788a14e5' };
        var update = { };
        ...
        setting.findByIdAndUpdate('5ec23c3d5afd4804788a14e5', update, function (err, setting) {
            if (err) return res.status(400).send({ success: false, message: 'Something went wrong' });
